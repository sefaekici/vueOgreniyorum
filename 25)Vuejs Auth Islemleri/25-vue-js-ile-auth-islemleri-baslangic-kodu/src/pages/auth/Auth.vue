...
<script>
    ...
    export default {
        ...
        methods: {
            onSubmit() {
                //axiosdan gelen veri return edilerek then işleminin çağılması sağlandı.Bu sayede then içerisinde push işlemi girişten sonra  yani cevap döndükten sonra çalışmış oldu.
                this.$store.dispatch("login",{user:this.user,isUser:this.isUser})
                .then(promise=>{
                    this.$router.push("/");
                })
            }
